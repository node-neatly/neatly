...
function Module(name, deps) {
	...
	const configQueue = this._configQueue = [];
	 ...
	 * @return {Object}              self
	 */
	function createQueuePusher(providerName, method, insertMethod, queue) {
		return function() {
			let args = arguments;
	...
	this.config = createQueuePusher('$injector', 'invoke', 'push', configQueue);
