 ...
 */
function createRunQueue(module) {
	return module.dependencies
		.map((sub) => createRunQueue(sub))
		.reduce((acc, res) => acc.concat(res), [])
		.concat(module._runQueue);
}
...
function bootstrap(module) {
	...
	let provides = createInvokeQueue(module)
	...
	return providerCache.$extProvider._runQueue('start')
		.then(() => createRunQueue(module).forEach((handler) => instanceInjector.invoke(handler)))
